...
/*   Updated: 2019/12/20 21:52:15 by iromero-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calcularobj(t_mapinfo *s)
{
		...
		{
			alturalinea = (int)(s->x / s->objdist);
			s->obstart = -alturalinea / 2 + (s->y / 2);
			if (s->obstart < 0)
				s->obstart = 0;
			s->obend = alturalinea /  2 + (s->y / 2);
			if (s->obend >= s->y)
				s->obend = s->y - 1;
			if (s->obstart >= s->obend)
				s->obstart = s->obend - s->h[7];
			alturalinea--;
		}
}
	...
	while (n < s->x)
	{
		...
		while (s->hit == 0)
		{
			...
			if (s->mapn[s->mapX][s->mapY] == 2)
			{
				...
				s->objposx = s->mapX;
				if (s->side == 0)
					s->objdist = (s->mapX - s->posX + (1 - s->stepX) / 4) / s->rayDirX;
				else
					s->objdist = (s->mapY - s->posY + (1 - s->stepY) / 4) / s->rayDirY;
			}
